...
function check_cookies() {
  ...
  local error_message=$(curl -s -b "$COOKIE" "https://issues.liferay.com/rest/auth/1/session" | jq -r '.errorMessages[0]')
...
function check_for_errors() {
  ...
  local error_message=$(jq -r '.message' <<< "$json_response")

  if [[ "$error_message" != "null" ]]; then
    local detailed_error_message=$(jq -r '.errors[0].message' <<< "$json_response")
...
function fetch_jira_issue() {
  ...
  local response=$(curl -s -b "$COOKIE" "${JIRA_ISSUES_API}/${branch_name}?fields=summary,issuetype")
...
function add_assignees_and_labels_to_pr() {
  ...
  local response=$(curl -s -X PATCH -H "Authorization: token ${GITHUB_TOKEN}" -d "$data" "$endpoint")
...
function create_gh_pull_request() {
  ...
  local response=$(fetch_jira_issue)

  local issue_number=$(echo $response | jq -r '.key')
  local title=$(echo "$response" | jq -r '(.key + " " + .fields.summary)')
  ...
  local gh_pr_response=$(curl -s -X POST -H "Authorization: token ${GITHUB_TOKEN}" -d "$data" "$endpoint")
...
function main() {
  ...
  local base_branch
  local base_user
  local head="$(get_github_user):$(get_branch_name)"
  local labels
  ...
  local pr_response=$(create_gh_pull_request "$base_user" "$base_branch" "$head")

  local pr_error_message=$(check_for_errors "$pr_response")

  if [[ -n "$pr_error_message" ]]; then
      echo "$pr_error_message"
  else
    local pr_id=$(jq -r '.number' <<< "$pr_response")
    local pr_url=$(jq -r '.html_url' <<< "$pr_response")
    local pr_title=$(jq -r '.title' <<< "$pr_response")

    echo "Pull request successfully submitted"
    echo "$pr_title"
    echo "$pr_url"

    local issue_response=$(add_assignees_and_labels_to_pr "$base_user" "$pr_id" "$assignees" "$labels")

    local issue_error_message=$(check_for_errors "$issue_response")
